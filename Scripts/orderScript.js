...
document.getElementById("location").setAttribute("placeholder", currentLocation);
...
function loadData() {
  ...
  resFetch.then(res=> res.json() )
  .then((res)=>
  {
    console.log("+++", res)
    data = res;
    document.getElementById("curPage").innerText = "/home/Orders/" + data["name"];
...
function menuHandle(menuItem) {
  ...
  .then((res)=>
  {
    ...
    for (let i = 0; i < reviewData.user_reviews.length; i++) {
      ...
      img.setAttribute("style", "border-radius: 50%; border: 1px solid black;");
...
setTimeout(()=>{
loadData();
menuHandle(document.getElementsByClassName("menuItem")[0]);}, 1000);
